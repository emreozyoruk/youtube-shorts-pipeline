...
def cmd_upload(args):
    ...
    # Upload
    privacy = getattr(args, "privacy", None) or os.environ.get("VERTICALS_PRIVACY", "private")
    if force or not state.is_done("upload"):
        url = upload_to_youtube(video_path, draft, srt_path, lang, thumb_path, privacy=privacy)
...
def cmd_run(args):
    ...
    skip_upload = os.environ.get("VERTICALS_SKIP_UPLOAD", "") == "1"
    if skip_upload:
        print(f"\n  Done! Video saved: {video_path}")
        print(f"  Upload skipped (disabled in GUI)")
        return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Docs: https://github.com/emreozyoruk/youtube-shorts-pipeline",
    )
    ...
    p_draft.add_argument("--lang", default="en", choices=["en", "hi", "es", "pt", "de", "fr", "ja", "ko", "tr"])

    # produce
    p_produce = sub.add_parser("produce", help="Generate video from draft")
    p_produce.add_argument("--draft", required=True)
    p_produce.add_argument("--lang", default="en", choices=["en", "hi", "es", "pt", "de", "fr", "ja", "ko", "tr"])
    ...
    p_upload.add_argument("--lang", default="en", choices=["en", "hi", "es", "pt", "de", "fr", "ja", "ko", "tr"])
    ...
    p_run.add_argument("--lang", default="en", choices=["en", "hi", "es", "pt", "de", "fr", "ja", "ko", "tr"])
