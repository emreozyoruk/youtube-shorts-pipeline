"""Multi-provider b-roll generation + Ken Burns animation."""
...
def generate_broll(prompts: list, out_dir: Path, image_provider: str = "auto") -> list[Path]:
    """Generate 3 b-roll frames via Gemini or OpenAI DALL-E, with fallback."""
    # Determine provider
    if image_provider == "openai":
        ...
        # Auto-detect: try config.json image_provider setting
        cfg_path = Path.home() / ".verticals" / "config.json"
        if cfg_path.exists():
            cfg = json.loads(cfg_path.read_text())
    ...
    for i, prompt in enumerate(prompts[:3]):
        ...
        log(f"Generating b-roll frame {i+1}/3 via {provider.upper()}...")

        try:
            if provider == "openai":
                api_key = _get_openai_key()
                _generate_image_openai(prompt, out_path, api_key)
            else:
